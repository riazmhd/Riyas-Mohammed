...
export const UploadsSidebar: React.FC<UploadsSidebarProps> = ({ data, isOpen, onClose }) => {
          ...
          <div className="flex-1 p-4 overflow-y-auto">
            {data.posts.length > 0 ? (
              <ul className="space-y-3">
                {data.posts.map((post) => (
                  <li key={post.id} className="bg-white/40 p-3 rounded-lg border border-white/30 group">
